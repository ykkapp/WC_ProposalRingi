...
const FaqItem = ({ item, level = 0, searchTerm }) => {
    ...
    // Leaf Node (Link)
    return (_jsx("div", { className: `border-b border-slate-200 last:border-b-0 ${levelStyles.bg}`, children: _jsxs("div", { className: "flex justify-between items-center py-3 pr-6 gap-2", style: { paddingLeft: levelStyles.paddingLeft }, children: [_jsx("span", { className: `${levelStyles.textSize} text-slate-700`, children: getHighlightedText(item.title, searchTerm) }), _jsxs("div", { className: "flex-shrink-0 flex items-center gap-2", children: [item.geminiUrl ? (_jsxs("a", { href: item.geminiUrl, target: "_blank", rel: "noopener noreferrer", className: "flex items-center text-white hover:text-white font-semibold text-xs sm:text-sm bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 rounded-full px-3 py-1.5 transition-all duration-150 shadow-sm", children: [_jsx("span", { children: "Gemini" }), _jsx(LinkIcon, {})] })) : (_jsx("span", { title: "\u30EA\u30F3\u30AF\u672A\u8A2D\u5B9A", "aria-disabled": "true", className: "flex items-center text-slate-400 font-semibold text-xs sm:text-sm bg-slate-100 rounded-full px-3 py-1.5 cursor-not-allowed select-none", children: _jsx("span", { children: "Gemini" }) })), item.link && (_jsxs("a", { href: (_b = item.url) !== null && _b !== void 0 ? _b : "#", onClick: !item.url ? (e) => e.preventDefault() : undefined, target: item.url ? "_blank" : undefined, rel: item.url ? "noopener noreferrer" : undefined, className: "flex items-center text-indigo-600 hover:text-indigo-800 font-semibold text-xs sm:text-sm bg-indigo-50 hover:bg-indigo-100 rounded-full px-3 py-1.5 transition-colors duration-150", children: [_jsx("span", { className: "truncate", children: getHighlightedText(item.link, searchTerm) }), _jsx(LinkIcon, {})] }))] })] }) }));
};
